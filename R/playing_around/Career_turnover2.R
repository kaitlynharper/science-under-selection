...
career_turnover2 <- function(sim_env) {
  ...
  career_contribution <- contribution_sums[match(active_researcher_ids, rownames(contribution_sums)), 1]
  career_contribution[is.na(career_contribution)] <- 0
  
  #### Identify agents with lowest percentile of career_contribution ####
  threshold <- quantile(career_contribution, probs = sim_env$career_turnover_selection_rate, na.rm = TRUE)
  ...
  new_rep_probs <- sample(sim_env$agents[surviving_indices, "replication_probability"], n_retire, replace = TRUE) + 
