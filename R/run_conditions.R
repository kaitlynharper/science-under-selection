...
initial_selection_condition_list <- c(0, 0, 1, 1, 1)
switch_conditions_at_list <- c(NA, NA, NA, NA, 200)
publication_bias_list <- c(0, 1, 0, 1, 1)
n_timesteps_list <- c(350, 350, 350, 350, 700)
...
for (i in 1:length(initial_selection_condition_list)) {
  ...
  ggsave(here("output", paste0("selection", initial_selection_condition, "_switch", switch_conditions_at, "_pub", publication_bias, "_t", n_timesteps, ".png")), patchwork_withtitle, width = 30, height = 10)
}
