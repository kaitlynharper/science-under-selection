...
assign_effects <- function(sim_env, verbose = FALSE) {
  ...
  # grab info once and store as vectors for faster access
  ...
  # filter studies by publication status and completion time
  # save published completed to env to be used later for replication studies
  ...
  # AVAILABLE EFFECTS FOR ORIGINALS
  ...
  # identify unavailable effect_ids
  taken_effect_ids <- c(
  ...
  # need max effect id for setting up new effects matrix block
  ...
  # AVAILABLE EFFECTS FOR REPLICATIONS
  # Effects that have already been completed and published at least once. Then, when
  ...
  # check if there are enough available effects for replication
  # if not, convert excess to originals
  if (sum(sim_env$is_replication) > length(published_completed_effect_ids)) {
    # calculate excess replications
    excess_replications <- sum(sim_env$is_replication) -
      sum(!is.na(published_completed_effect_ids))
    # identify indices of replications to convert to originals
    ...
    # convert replications to originals
  ...
  # In the burn-in period, set all studies to originals (=0)
  if (sim_env$timestep <= sim_env$burn_in_period) {
    sim_env$new_studies[, "study_type"] <- 0
  }

  # check if there are enough original effects
  # if not, append a new block (double effects) and recompute pool
  ...
  if (n_original > length(available_original_effects)) {
    ...
    # create new rows for new effects
    new_rows <- cbind(
      effect_id = new_ids,
      timestep = 0, # as if they always existed
  ...
  # assign effect_ids to original studies (sample without replacement)
  ...
  # identify published completed and in-progress studies
  pub <- sim_env$published_completed | published_in_progress
  # count publications by effect_id by counting non-NA effect_ids
  # (tabulate: counts the number of occurrences of each integer in a vector
  # resulting in a vector of counts for each effect_id)
  ...
  # assign effect_ids to replication studies from ordered list
...
# After effect assignment, cache info used by later steps
prepare_information <- function(sim_env) {

  # published completed originals (only needed if there are replications)
  if (any(sim_env$is_replication)) {
    # identify published completed originals
    pub_orig <- sim_env$published_completed &
      sim_env$studies[, "study_type"] == 0 
    # grab them from the studies matrix
    pub_orig_studies <- sim_env$studies[pub_orig, , drop = FALSE]

    # match new studies to published completed originals
    orig_match <- match(
      sim_env$new_studies[, "effect_id"],
      pub_orig_studies[, "effect_id"]
    )

    # grab estimated means and p-values from published completed originals
  ...
  # latest belief row for each assigned effect (via latest_row tracker index)
  effect_rows <- sim_env$latest_row[sim_env$new_studies[, "effect_id"]]

  # grab true means, variances, and existing posteriors (current priors)from effects matrix
  # save all to env for later use
...
#### Function: determine_sample_sizes ####
determine_sample_sizes <- function(sim_env) {
  ...
  # if dynamic replication sample sizes disabled or no replications, all done
  if (
  ...
  # FOR REPLICATIONS: CALCULATE POWER-BASED SAMPLE SIZES
  # grab original means and p-values
  orig_means <- sim_env$orig_estimated_mean[sim_env$is_replication]
  orig_pvals <- sim_env$orig_p_value[sim_env$is_replication]

  # grab reference effects: original effect if significant, otherwise 0.3 default
  ...
  # only calculate once for each unique delta (so the default 0.3 is only calculated once)
  ...
  # assign sample sizes to replication studies based on unique delta values
  rep_sample_sizes <- unique_n[match(reference_effects, unique_d)]

  # add these sample sizevalues to the new studies matrix
  sim_env$new_studies[sim_env$is_replication, "sample_size"] <- rep_sample_sizes
}

#### Function: determine_study_durations ####
determine_study_durations <- function(sim_env) {
  ...
  # identify indices of researchers with new studies
  agent_indices <- match(
    sim_env$new_studies[, "researcher_id"],
    sim_env$agents[, "researcher_id"]
  )
  # update their timestep_next_paper
  sim_env$agents[agent_indices, "timestep_next_paper"] <- sim_env$new_studies[,
    "timestep_completed"
  ]
}

#### Function: generate_study_results ####
generate_study_results <- function(sim_env) {
  
  # grab number of studies and sample sizes
  n_studies <- nrow(sim_env$new_studies)
  sample_sizes <- sim_env$new_studies[, "sample_size"]

  # simulate observed t-statistics using noncentral t-distribution
  # ncp = noncentrality parameter (when = 0, this is a central t-distribution)
  ncp <- sqrt(sample_sizes / 2) * sim_env$true_means
  # degrees of freedom is 2 * (sample_sizes per group - 1)
  df <- 2 * (sample_sizes - 1)
  # simulate t-statistics from noncentral t-distribution
  ...
  # store results in new studies matrix
...
kl_norm <- function(mu0, sd0, mu1, sd1) {
  # KL divergence from P(mu0, sd0) to Q(mu1, sd1)
  # directional such that predictions from P are being compared to reality Q
...
prepare_bayesian_data <- function(sim_env) {
  ...
  # grab likelihood means and variances from new studies matrix
  ...
  # save new posteriors in env for later use
  sim_env$new_posterior_means <- posterior_means
  sim_env$new_posterior_vars <- posterior_vars
}

#### Function: calculate_novelty_contribution ####
calculate_novelty_contribution <- function(sim_env) {
  # information gain = KL(new posterior || old posterior or current prior)
  novelty <- kl_norm(
  ...
  # save novelty contribution to new studies matrix
  sim_env$new_studies[, "novelty_contribution"] <- novelty
}

#### Function: calculate_truth_contribution ####
# = log posterior density at true effect - log prior density at true effect
calculate_truth_contribution <- function(sim_env) {

  if (sim_env$truth_contribution_method == "savage_dickey") {
    # dnorm(x, mean, sd, log = TRUE) returns log density
    # of specified normal distribution at x
    ...
    # truth contribution = log posterior at true - log prior at true
    truth <- log_posterior_at_true - log_prior_at_true
  } else {
    # kl tells us how far the prior and posterior are from the true effect
    ...
    # truth contribution = KL(true effect || prior) - KL(true effect | posterior)
    truth <- kl_prior_to_true - kl_posterior_to_true
  }
  # save truth contribution to new studies matrix
  sim_env$new_studies[, "truth_contribution"] <- truth
}

#### Function: update_effects_beliefs ####
update_effects_beliefs <- function(sim_env) {
  # grab new published studies
  is_published <- sim_env$new_studies[, "publication_status"] == 1
  n_published <- sum(is_published)

  if (n_published == 0) {
    return() #finish if no new published studies
  }

  # create new effect rows for each new published study
  ...
  # append new rows to effects matrix
  old_n <- nrow(sim_env$effects) # get number of existing rows
  sim_env$effects <- rbind(sim_env$effects, new_effect_rows) # append new rows
  sim_env$latest_row[new_effect_rows[, "effect_id"]] <- (old_n + 1):(old_n + 
    n_published) # update latest_row tracker to keep track of the latest belief row for each effect
}
