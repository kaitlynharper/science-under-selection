...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      autonumericInput(
        inputId = "true_effect_mean",
        label = "True Effect Mean:",
        value = 0.0,
        decimalCharacter = ".",
        digitGroupSeparator = ",",
        decimalPlaces = 2,
      ...
      autonumericInput(
        inputId = "true_effect_sd",
        label = "True Effect SD:",
        value = 0.01,
        decimalCharacter = ".",
        digitGroupSeparator = ",",
        decimalPlaces = 2,
...
server <- function(input, output, session) {
  ...
  observeEvent(input$scenario, {
    ...
    if (is.null(defaults)) {
      return(NULL)
    } # Safety check: exit if scenario not found

    # Update numeric fields; users can still change them manually afterwards
    shinyWidgets::updateAutonumericInput(
      session,
      "true_effect_mean",
      value = defaults$true_effect_mean
    )
    shinyWidgets::updateAutonumericInput(
      session,
      "true_effect_sd",
      value = defaults$true_effect_sd
    )
    shinyWidgets::updateAutonumericInput(
      session,
      "prior_mean",
      value = defaults$prior_mean
    )
    shinyWidgets::updateAutonumericInput(
      session,
      "prior_sd",
      value = defaults$prior_sd
    )
    shinyWidgets::updateAutonumericInput(
      session,
      "study_effect",
      value = defaults$study_effect
    )
    shinyWidgets::updateAutonumericInput(
      session,
      "study_se",
      value = defaults$study_se
    )
  })
  ...
  output$dist_plot <- renderPlot({
    ...
    polygon(
      c(x_plot, rev(x_plot)),
      c(rep(0, length(y_true)), rev(y_true)),
      col = adjustcolor("black", alpha.f = 0.2),
      border = NA
    )
    polygon(
      c(x_plot, rev(x_plot)),
      c(rep(0, length(y_prior)), rev(y_prior)),
      col = adjustcolor("blue", alpha.f = 0.2),
      border = NA
    )
    polygon(
      c(x_plot, rev(x_plot)),
      c(rep(0, length(y_likelihood)), rev(y_likelihood)),
      col = adjustcolor("gray", alpha.f = 0.2),
      border = NA
    )
    polygon(
      c(x_plot, rev(x_plot)),
      c(rep(0, length(y_posterior)), rev(y_posterior)),
